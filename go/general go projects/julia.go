...
func Julia(f ComplexFunc, n int) image.Image {
	...
	s := float64(n / 4)
	var wg sync.WaitGroup //Create a wait-group for the computations.
	wg.Add(bounds.Max.X - bounds.Min.X)
	for i := bounds.Min.X; i < bounds.Max.X; i++ {
		go func(i int) { //Create a go-routine for each work unit.
...
func init() {
	...
	fmt.Println("Number of CPU:s for Julia: ", numcpu)
}